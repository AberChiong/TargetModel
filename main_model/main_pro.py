...
# 通过无效化xl，只保留xp设置阴性对照实验，来评估模型性能，探究主模型整体的鲁棒性。
# 作者：常怡彬
# 最后更新：2023年10月18日
...
def train(device, num_epochs, net, optimizer, batch_size=128, shuffle=True, batch_print_interval=16):  
    ...
    for epoch in tqdm(range(num_epochs)):
        ...
        for file_idx in range(6):
            ...
            for batch_idx, (inputsp, _, labels) in enumerate(data_loader): # 丢弃原本的xl信息
                inputsp = inputsp.permute(0, 4, 1, 2, 3)
                inputsl = torch.zeros_like(inputsp) # 重新生成形状等于xp的零张量xl
